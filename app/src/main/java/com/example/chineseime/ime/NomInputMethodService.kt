...
class NomInputMethodService : InputMethodService(), KeyboardController.Listener {
    ...
    override fun onFinishInput() { state.reset(); input.finishComposing(); super.onFinishInput() }
    ...
    private fun enqueueSentenceQuery() {
        ...
        val task=Runnable {
            executor.execute {
                try {
                    val result=engine.query(snapshot,8)
                    mainHandler.post {
        ...
        pendingQuery=task
        mainHandler.postDelayed(task,80L)
    }
